...
class OrderEmbedder(nn.Module):
    ...
    def criterion(self, pred, intersect_embs, labels):
        ...
        """
        emb_as, emb_bs = pred
        e = torch.sum(torch.max(cudavar(self.av, torch.zeros_like(emb_as)), emb_bs - emb_as)**2, dim=1)

        margin = self.margin
        e[labels == 0] = torch.max(cudavar(self.av, torch.tensor(0.0)), margin - e)[labels == 0]
...
class SkipLastGNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, args):
        ...
        self.post_mp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, args.post_mp_dim), nn.ReLU(),
            nn.Linear(args.post_mp_dim, hidden_dim))
